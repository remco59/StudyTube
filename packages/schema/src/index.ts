...
const chapterIntroSceneSchema = z.object({
  ...sceneBaseShape,
...
const kineticTextSceneSchema = z.object({
  ...sceneBaseShape,
...
const definitionSceneSchema = z.object({
  ...sceneBaseShape,
...
const processSceneSchema = z.object({
  ...sceneBaseShape,
...
const flowchartSceneSchema = z.object({
  ...sceneBaseShape,
...
const iconSceneItemSchema = z.object({
  icon: z.string().min(1).max(100),
  label: z.string().min(1).max(100),
  detail: z.string().max(180).optional(),
}).strict();

const iconSceneSchema = z.object({
  ...sceneBaseShape,
...
const documentSceneSchema = z.object({
  ...sceneBaseShape,
...
const documentHighlightSceneSchema = z.object({
  ...sceneBaseShape,
...
const visualGagSceneSchema = z.object({
  ...sceneBaseShape,
...
const bulletRevealSceneSchema = z.object({
  ...sceneBaseShape,
  type: z.literal("bulletReveal"),
  visual: z.object({
    title: z.string().max(140).optional(),
    points: z.array(z.string().min(1).max(180)).min(2).max(6),
  }).strict(),
...
const annotationSchema = z.object({
  ...
  targetX: z.number().min(0).max(100).optional(),
  targetY: z.number().min(0).max(100).optional(),
}).strict();

const annotatedImageSceneSchema = z.object({
  ...sceneBaseShape,
  type: z.literal("annotatedImage"),
  visual: z.object({
    assetId: idSchema,
    fit: z.enum(["contain", "cover"]).optional(),
    title: z.string().max(140).optional(),
...
const dataChartSceneSchema = z.object({
  ...sceneBaseShape,
  type: z.literal("dataChart"),
  visual: z.object({
    title: z.string().max(140).optional(),
    chartType: z.enum(["bar", "line", "donut"]),
    data: z.array(chartDatumSchema).min(2).max(8),
    unit: z.string().max(24).optional(),
    sourceLabel: z.string().max(120).optional(),
  }).strict(),
}).strict();

const matrixQuadrantSchema = z.object({
  title: z.string().min(1).max(80),
  detail: z.string().max(180).optional(),
}).strict();

const matrixSceneSchema = z.object({
  ...sceneBaseShape,
  type: z.literal("matrix"),
  visual: z.object({
    title: z.string().max(140).optional(),
    xAxis: z.object({low: z.string().max(60), high: z.string().max(60)}).strict().optional(),
    yAxis: z.object({low: z.string().max(60), high: z.string().max(60)}).strict().optional(),
    quadrants: z.object({
      topLeft: matrixQuadrantSchema,
      topRight: matrixQuadrantSchema,
      bottomLeft: matrixQuadrantSchema,
      bottomRight: matrixQuadrantSchema,
    }).strict(),
  }).strict(),
}).strict();
...
const cycleSceneSchema = z.object({
  ...sceneBaseShape,
  type: z.literal("cycle"),
  visual: z.object({
    title: z.string().max(140).optional(),
    center: z.string().max(100).optional(),
    steps: z.array(cycleStepSchema).min(3).max(8),
  }).strict(),
}).strict();

const multipleChoiceOptionSchema = z.object({
  label: z.string().min(1).max(140),
  explanation: z.string().max(180).optional(),
}).strict();

const multipleChoiceSceneSchema = z.object({
  ...sceneBaseShape,
  ...
  visual: z.object({
    ...
    revealAfterSeconds: z.number().min(0).max(30).optional(),
  }).strict(),
}).strict();
...
const workedExampleSceneSchema = z.object({
  ...sceneBaseShape,
  type: z.literal("workedExample"),
  visual: z.object({
    title: z.string().max(140).optional(),
    problem: z.string().min(1).max(260),
    steps: z.array(workedExampleStepSchema).min(1).max(5),
    result: z.string().max(220).optional(),
  }).strict(),
}).strict();
...
const hierarchySceneSchema = z.object({
  ...sceneBaseShape,
  type: z.literal("hierarchy"),
  visual: z.object({
    title: z.string().max(140).optional(),
    direction: z.enum(["topDown", "bottomUp"]).optional(),
    levels: z.array(hierarchyLevelSchema).min(2).max(6),
  }).strict(),
}).strict();
...
export const studyTubeSceneSchema = z.discriminatedUnion("type", [
  ...
  recapSceneSchema,
  bulletRevealSceneSchema,
  annotatedImageSceneSchema,
  dataChartSceneSchema,
  matrixSceneSchema,
  cycleSceneSchema,
...
export const studyTubeProjectSchema = z.object({
  ...
  project.chapters.forEach((chapter, chapterIndex) => {
    if (chapterIds.has(chapter.id)) {
      context.addIssue({code: "custom", path: ["chapters", chapterIndex, "id"], message: `Duplicate chapter id: ${chapter.id}`});
    }
    chapterIds.add(chapter.id);

    chapter.scenes.forEach((scene, sceneIndex) => {
      if (sceneIds.has(scene.id)) {
        context.addIssue({code: "custom", path: ["chapters", chapterIndex, "scenes", sceneIndex, "id"], message: `Duplicate scene id: ${scene.id}`});
      }
      ...
      if (scene.type === "flowchart") {
        ...
        scene.visual.edges.forEach((edge, edgeIndex) => {
          if (!nodeIds.has(edge.from)) {
            context.addIssue({code: "custom", path: ["chapters", chapterIndex, "scenes", sceneIndex, "visual", "edges", edgeIndex, "from"], message: `Unknown flowchart node: ${edge.from}`});
          }
          if (!nodeIds.has(edge.to)) {
            context.addIssue({code: "custom", path: ["chapters", chapterIndex, "scenes", sceneIndex, "visual", "edges", edgeIndex, "to"], message: `Unknown flowchart node: ${edge.to}`});
          }
        });
      }

      if (scene.type === "multipleChoice" && scene.visual.correctIndex >= scene.visual.options.length) {
...
export const formatStudyTubeValidationIssues = (error: z.ZodError): StudyTubeValidationIssue[] =>
...
export const parseStudyTubeProject = (input: unknown): StudyTubeProject => {
  ...
  if (!result.success) throw new StudyTubeValidationError(formatStudyTubeValidationIssues(result.error));
  return result.data;
};
