...
export const searchStockAssets=async(input:{query:string;kind:StockAssetKind;providers?:StockProviderName[];limitPerProvider?:number})=>{
  ...
  if(providers.length===0){
    const providerNames=input.kind==="video"?"Pixabay or Pexels":"Pixabay, Pexels or Unsplash";
    throw new StockAssetError(`No enabled ${input.kind} stock provider is configured. Add a ${providerNames} API key in Settings.`);
  }
  ...
  for(const provider of providers){
    try{
      ...
      failures.push(`${provider}: ${error instanceof Error?error.message:"search failed"}`);
    }
  }
...
export const publicStockSearchResult=(result:StockSearchResult):PublicStockSearchResult=>{
  const {downloadUrl,downloadLocation,...publicResult}=result;
  void downloadUrl;
...
const searchPixabay=async(query:string,kind:StockAssetKind,key:string,limit:number):Promise<StockSearchResult[]>=>{
  ...
  for(const raw of payload.hits??[]){
    ...
    if(kind==="image"){
      ...
      results.push({
        provider:"pixabay",providerId:id,kind:"image",previewUrl:text(hit.webformatURL)||downloadUrl,downloadUrl,
      ...
      const chosen=choosePixabayVideo(asRecord(hit.videos));
      if(!chosen)continue;
      results.push({
        provider:"pixabay",providerId:id,kind:"video",previewUrl:chosen.url,downloadUrl:chosen.url,width:chosen.width,height:chosen.height,
...
const searchPexels=async(query:string,kind:StockAssetKind,key:string,limit:number):Promise<StockSearchResult[]>=>{
  ...
  const rawItems=kind==="video"?payload.videos:payload.photos;
  ...
  for(const raw of items){
    ...
    if(kind==="image"){
      ...
      const downloadUrl=text(src?.large2x)||text(src?.large)||text(src?.original);
      if(!downloadUrl)continue;
      const creator=text(item.photographer);
      results.push({
        provider:"pexels",providerId:id,kind:"image",previewUrl:text(src?.large)||downloadUrl,downloadUrl,
        width:number(item.width),height:number(item.height),creator,creatorUrl:text(item.photographer_url),sourceUrl:text(item.url),alt:text(item.alt),
        attributionText:creator?`Photo by ${creator} on Pexels`:"Photo from Pexels",licenseLabel:"Pexels License",
      });
    }else{
      const chosen=choosePexelsVideo(Array.isArray(item.video_files)?item.video_files:[]);
      if(!chosen)continue;
      const user=asRecord(item.user);
      const creator=text(user?.name);
      results.push({
        provider:"pexels",providerId:id,kind:"video",previewUrl:text(item.image)||chosen.link,downloadUrl:chosen.link,
        width:chosen.width??number(item.width),height:chosen.height??number(item.height),durationSeconds:number(item.duration),
        creator,creatorUrl:text(user?.url),sourceUrl:text(item.url),alt:`${query} stock video`,
        attributionText:creator?`Video by ${creator} on Pexels`:"Video from Pexels",licenseLabel:"Pexels License",
      });
    }
  }
  return results;
...
const searchUnsplash=async(query:string,key:string,limit:number):Promise<StockSearchResult[]>=>{
  ...
  for(const raw of payload.results??[]){
    ...
    const downloadUrl=text(urls?.full)||text(urls?.regular);
    const previewUrl=text(urls?.regular)||downloadUrl;
    const downloadLocation=text(links?.download_location);
    if(!downloadUrl||!previewUrl||!downloadLocation)continue;
    const creator=text(user?.name);
    results.push({
      provider:"unsplash",providerId:id,kind:"image",previewUrl,downloadUrl,downloadLocation,
      width:number(item.width),height:number(item.height),creator,creatorUrl:text(userLinks?.html),sourceUrl:text(links?.html),
      alt:text(item.alt_description)||text(item.description),attributionText:creator?`Photo by ${creator} on Unsplash`:"Photo from Unsplash",licenseLabel:"Unsplash License",
    });
  }
  return results;
};
...
const choosePixabayVideo=(videos:Record<string,unknown>|undefined)=>{
  ...
  for(const key of ["medium","large","small","tiny"]){
    const candidate=asRecord(videos[key]);
    const url=text(candidate?.url);
    if(url)return {url,width:number(candidate?.width),height:number(candidate?.height)};
  }
  return null;
};

const choosePexelsVideo=(files:unknown[])=>{
  const candidates:Array<{link:string;width?:number;height?:number}>=[];
  for(const raw of files){
    const file=asRecord(raw);
    if(!file||!text(file.file_type)?.includes("mp4"))continue;
    const link=text(file.link);
    if(!link)continue;
    candidates.push({link,width:number(file.width),height:number(file.height)});
  }
  candidates.sort((a,b)=>Math.abs(1920-(a.width??0))-Math.abs(1920-(b.width??0)));
...
const extensionFor=(contentType:string,kind:StockAssetKind,url:string)=>{
  ...
  const match=pathname.match(/\.([a-z0-9]{2,5})$/u);
  if(match&&["jpg","jpeg","png","webp","mp4","webm"].includes(match[1]!))return match[1]==="jpeg"?"jpg":match[1]!;
