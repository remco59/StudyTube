...
export const buildChatGptPrompt=({targetDurationMinutes,language,scope,useAssets=false,assetTypes=["web-images","generated-images"],assetAmount="some",teaching=defaultPromptTeachingConfig}:PromptOptions)=>{
  ...
  const assetSceneRules=useAssets
    ?`Use these scene types only when the corresponding asset is present in the project package:\n\nimage\n{"assetId":"existing image asset ID","variant":"full | split-text | split-image","layout":"image-left | image-right","splitRatio":"40/60 | 50/50 | 60/40","fit":"contain | cover","title":"optional","text":"optional","caption":"optional","secondaryAssetId":"required for split-image","secondaryFit":"contain | cover","secondaryCaption":"optional"}\nOmit variant or use full for the classic full-width image scene. Use split-text when an image supports a concise explanation; it requires at least title or text and layout controls which side contains the image. Use split-image for side-by-side comparison and always provide secondaryAssetId referencing another packaged image. Prefer fit=contain for portrait, square, screenshot, diagram or infographic assets so important content is not cropped. splitRatio describes the first pane versus the second pane.\n\nannotatedImage\n{"assetId":"existing image asset ID","fit":"contain | cover","title":"optional","annotations":[{"label":"required","x":50,"y":50,"targetX":"optional 0-100","targetY":"optional 0-100"}],"caption":"optional"}\nCoordinates are percentages from the top-left. Keep annotations sparse, normally 1-5, and place labels so they do not overlap important image content.\n\ndocument\n{"assetId":"existing document asset ID","page":1,"caption":"optional"}\n\ndocumentHighlight\n{"assetId":"existing document asset ID","page":1,"highlightText":"required","caption":"optional"}`
...
${useAssets?"Use annotatedImage when labels or callouts make an image teach something that the image alone would not communicate. Prefer image variant split-text when the image supports an explanation, especially for portrait, square, screenshot, diagram or infographic assets that would crop badly in a full-width frame. Prefer split-image for meaningful visual comparisons or two complementary examples. Use full image scenes when the image itself should dominate the scene. Use image/document scenes according to the requested asset amount, but only when the asset genuinely improves the explanation or visual variety.":"Create visual variety with the structured scene types above instead of relying on external assets."}
