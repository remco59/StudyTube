      ...
      {activeTab==="create"?<>
        ...
        <div className="jobsWorkspace">
          <section className="jobsPanel">
            ...
            {jobs.length===0?<div className="jobsEmpty"><strong>No jobs yet.</strong><span>Create your first video from the Create tab.</span><button type="button" className="primaryButton compactButton" onClick={()=>setActiveTab("create")}>Create video</button></div>:<div className="jobsList">{jobs.map((item)=><button type="button" className={`jobRow${managedJobId===item.jobId?" selected":""}`} key={item.jobId} onClick={()=>selectManagedJob(item)}>{item.thumbnailPath?<img className="jobThumbnail" src={`/api/jobs/${item.jobId}/thumbnail`} alt=""/>:null}<div className="jobIdentity"><strong>{item.projectTitle??"StudyTube render"}</strong><span>{item.createdAt?formatJobDate(item.createdAt):item.jobId}</span></div><div className="jobState"><span className={`jobStatePill ${stateClass(item.state)}`}>{jobStatusLabel(item)}</span>{!isTerminal(item.state)?<span className="jobProgress">{Math.round(item.progress*100)}%</span>:null}</div></button>)}</div>}
          </section>

          {managedJob?<section className="jobManager">
            <div className="jobManagerHeader">{managedJob.thumbnailPath?<img className="jobManagerThumbnail" src={`/api/jobs/${managedJob.jobId}/thumbnail`} alt=""/>:null}<div><p className="eyebrow">Selected job</p><h2>{managedJob.projectTitle??"StudyTube render"}</h2><span className="jobIdText">{managedJob.jobId}</span><span className="jobEngineText">{renderEngineLabel(managedJob.renderEngine??"cpu")}</span>{managedJob.ttsProvider?<span className="jobEngineText">{ttsProviderLabel(managedJob.ttsProvider)}</span>:null}</div><span className={`jobStatePill ${stateClass(managedJob.state)}`}>{jobStatusLabel(managedJob)}</span></div>
