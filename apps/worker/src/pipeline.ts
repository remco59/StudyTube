...
export const runStudyTubeJob=async(options:RunStudyTubeJobOptions,deps:PipelineDependencies={}):Promise<StudyTubeJobResult>=>{
  ...
  try{
    ...
    if(options.signal?.aborted){await log("job.cancelled","Render cancelled by user").catch(()=>undefined);await update("cancelled",status.progress,{error:undefined}).catch(()=>undefined);await Promise.all([statusQueue.catch(()=>undefined),logQueue.catch(()=>undefined)]);throw new StudyTubeJobCancelledError(jobId,paths.root,error);}
    const message=error instanceof Error?error.message:String(error);await log("job.failed",message).catch(()=>undefined);await update("failed",status.progress,{error:message}).catch(()=>undefined);await Promise.all([statusQueue.catch(()=>undefined),logQueue.catch(()=>undefined)]);throw new StudyTubeJobError(jobId,paths.root,error);
  }
};
