...
export const extractSegment=async(sourcePath:string,startFrame:number,frameCount:number,fps:number,destPath:string,signal?:AbortSignal):Promise<void>=>{
  ...
  await execFileAsync(ffmpegBinary(),["-y","-i",sourcePath,"-ss",startSeconds,"-t",durationSeconds,"-map","0","-c:v","libx264","-preset","veryfast","-crf","18","-c:a","aac","-avoid_negative_ts","make_zero",destPath],{maxBuffer:16_000_000,timeout:ffmpegTimeoutMs(),signal});
};
...
export const concatenateSegments=async(segmentPaths:string[],destPath:string,signal?:AbortSignal):Promise<void>=>{
  ...
  await execFileAsync(ffmpegBinary(),["-y","-f","concat","-safe","0","-i",listPath,"-c","copy",destPath],{maxBuffer:16_000_000,timeout:ffmpegTimeoutMs(),signal});
};
