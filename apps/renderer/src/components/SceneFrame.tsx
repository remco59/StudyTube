...
export const SceneFrame=({normalizedScene,project,hasCaptions=false}:SceneFrameProps)=>{
  ...
  const contentPadding=immersive?`0 0 ${captionReserve}px`:`${spacing.md}px 0 ${spacing.md+captionReserve}px`;

  return <AbsoluteFill style={{background:`radial-gradient(circle at 84% 18%, ${colors.accentSoft} 0%, transparent 30%), linear-gradient(135deg, ${colors.canvas} 0%, ${colors.canvasSoft} 100%)`,overflow:"hidden",paddingTop:video.safeArea.top,paddingRight:video.safeArea.right,paddingBottom:video.safeArea.bottom,paddingLeft:video.safeArea.left}}>
