...
export const StudyTubeComposition = ({project}: StudyTubeCompositionProps) => {
  return (
    <AbsoluteFill
      style={{
    ...
    >
      {project.chapters.flatMap((chapter) =>
        chapter.scenes.map((normalizedScene) => (
          <Sequence
            key={normalizedScene.scene.id}
            from={normalizedScene.startFrame}
            durationInFrames={normalizedScene.durationInFrames}
            name={`${chapter.title} / ${normalizedScene.scene.id}`}
          >
            <SceneFrame
